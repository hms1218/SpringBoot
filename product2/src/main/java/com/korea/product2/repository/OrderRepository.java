...
public interface OrderRepository extends JpaRepository<OrderEntity, Integer>{
	...
	//총 결제 금액(주문개수 x 상품의 가격) 도 조회를 해보자
	//@ManyToOne으로 설정된 관계 매핑 덕분에, OrderEntity에서 product필들르 통해
	//ProductEntity에 접근할 수 있다.
	//JPA는 JPQL 쿼리를 분석하고, OrderEntity에서 ProductEntity와의 관계가 설정된 것을
	//감지하면 자동으로 SQL조인을 생성한다.
